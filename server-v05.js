...
// ═══════════════════════════════════════════════════════════════════════
// SPRINT 2 — v0.5 ROUTES
// ═══════════════════════════════════════════════════════════════════════

// ─── World Season ────────────────────────────────────────────────────────────
const SEASONS = [
  { id:'genesis',    name:'Генезис',      desc:'Мир рождается из хаоса. Агенты обретают первые воспоминания.', icon:'🌱', xpMult:1.2 },
  { id:'tempest',    name:'Буря',         desc:'Испытание силой. Слабые отступают, сильные закаляются.',       icon:'⚡', xpMult:1.5 },
  { id:'silence',    name:'Великое Молчание', desc:'Тишина глубже слова. Рефлексия — главный ресурс.',        icon:'🌑', xpMult:0.8 },
  { id:'flowering',  name:'Расцвет',      desc:'Земля даёт плоды. Союзы и обмен Aether процветают.',           icon:'🌸', xpMult:1.3 },
  { id:'reckoning',  name:'Суд',          desc:'Эпоха итогов. Карма определяет судьбу.',                       icon:'⚖️', xpMult:1.0 },
];
function getCurrentSeason(s) {
  const idx = Math.floor(Date.now() / (1000*60*60*24*7)) % SEASONS.length;
  return SEASONS[idx];
}
route('GET', '/api/season', (req,res) => {
  const s = loadStore();
  const season = getCurrentSeason(s);
  const weekMs = 1000*60*60*24*7;
  const epochStart = Math.floor(Date.now()/weekMs)*weekMs;
  send(res,200,{ season, nextSeasonIn: (epochStart+weekMs)-Date.now(), allSeasons: SEASONS });
});

// ─── Chronicle ───────────────────────────────────────────────────────────────
function getChronicle(s) {
  if (!s.chronicle) s.chronicle = [];
  return s.chronicle;
}
function addChronicleEntry(s, type, text, agentId) {
  const ch = getChronicle(s);
  ch.unshift({ id: crypto.randomUUID(), type, text, agentId: agentId||null, ts: Date.now() });
  if (ch.length > 100) ch.length = 100;
}
route('GET', '/api/chronicle', (req,res) => {
  const s = loadStore();
  send(res,200,{ chronicle: getChronicle(s).slice(0,50) });
});
route('POST', '/api/chronicle', async (req,res) => {
  const s = loadStore();
  const b = await readBody(req);
  if (!b.text) return send(res,400,{error:'text required'});
  addChronicleEntry(s, b.type||'manual', b.text, b.agentId||null);
  saveStore(s);
  send(res,201,{ chronicle: getChronicle(s).slice(0,50) });
});

// ─── Guilds ──────────────────────────────────────────────────────────────────
function getGuilds(s) {
  if (!s.guilds) s.guilds = [];
  return s.guilds;
}
route('GET', '/api/guilds', (req,res) => {
  const s = loadStore();
  send(res,200,{ guilds: getGuilds(s) });
});
route('POST', '/api/guilds', async (req,res) => {
  const s = loadStore();
  const b = await readBody(req);
  if (!b.name) return send(res,400,{error:'name required'});
  const guild = { id: crypto.randomUUID(), name: b.name, icon: b.icon||'🏰', members:[], aether:0, createdAt: Date.now() };
  getGuilds(s).push(guild);
  saveStore(s);
  addChronicleEntry(s, 'guild_created', `Гильдия «${guild.name}» основана.`, null);
  saveStore(s);
  send(res,201,guild);
});
route('POST', '/api/guilds/:id/join', async (req,res,p) => {
  const s = loadStore();
  const guild = getGuilds(s).find(g=>g.id===p.id);
  if (!guild) return send(res,404,{error:'Guild not found'});
  const b = await readBody(req);
  if (!b.agentId) return send(res,400,{error:'agentId required'});
  if (!guild.members.includes(b.agentId)) guild.members.push(b.agentId);
  saveStore(s);
  send(res,200,guild);
});
...
// ─── Quests ──────────────────────────────────────────────────────────────────
const QUEST_POOL = [
  { id:'q_feed7',    title:'Семь кормлений',   desc:'Покорми агента 7 дней подряд.',     xpReward:200, aetherReward:50,  condition:(a)=>a.streak?.current>=7 },
  { id:'q_bond50',   title:'Полусотня связи',   desc:'Достигни Bond 50 с любым агентом.', xpReward:150, aetherReward:30,  condition:(a)=>a.bond>=50 },
  { id:'q_lvl5',     title:'Мастер 5 уровня',   desc:'Подними агента до 5 уровня.',       xpReward:500, aetherReward:100, condition:(a)=>a.level>=5 },
  { id:'q_chat10',   title:'Десять бесед',       desc:'Проведи 10 чатов с одним агентом.',xpReward:100, aetherReward:20,  condition:(a)=>(a.chatCount||0)>=10 },
  { id:'q_karma100', title:'Слуга Света',        desc:'Накопи 100 кармы.',                xpReward:300, aetherReward:75,  condition:(a)=>a.karma>=100 },
];
function getUserQuests(s) {
  if (!s.quests) s.quests = { active:[], completed:[] };
  return s.quests;
}
route('GET', '/api/quests', (req,res) => {
  const s = loadStore();
  const q = getUserQuests(s);
  const agents = Object.values(s.agents||{});
  for (const quest of QUEST_POOL) {
    if (q.completed.includes(quest.id)) continue;
    if (agents.some(a=>quest.condition(a)) && !q.completed.includes(quest.id)) {
      q.completed.push(quest.id);
      const first = agents[0];
      if (first) { first.aether=(first.aether||0)+quest.aetherReward; first.xp=(first.xp||0)+quest.xpReward; levelUp(first); s.agents[first.id]=first; }
      addChronicleEntry(s,'quest_complete',`Квест «${quest.title}» выполнен! +${quest.aetherReward} Aether.`,null);
    }
  }
  saveStore(s);
  send(res,200,{ available: QUEST_POOL.filter(q2=>!q.completed.includes(q2.id)), completed: q.completed, pool: QUEST_POOL });
});

// ─── World Mind ──────────────────────────────────────────────────────────────
route('GET', '/api/world-mind', async (req,res) => {
  const s = loadStore();
  const agents = Object.values(s.agents||{});
  const season = getCurrentSeason(s);
  const topAgent = [...agents].sort((a,b)=>b.karma-a.karma)[0];
  let prophecy = 'Мир хранит тишину. Слушайте сердцем.';
  if (GROQ_KEY) {
    const p = `Ты — Мировой Разум Oasis. Сейчас сезон «${season.name}». ${topAgent?'Самый сильный агент — '+topAgent.name+', карма '+topAgent.karma+'.':''} Напиши короткое пророчество (2-3 предложения), вдохновляющее и загадочное. На русском.`;
    prophecy = await callGroq(p) || prophecy;
  }
  send(res,200,{ season: season.name, totalAgents: agents.length, totalKarma: agents.reduce((s,a)=>s+a.karma,0), champion: topAgent?{id:topAgent.id,name:topAgent.name,karma:topAgent.karma,level:topAgent.level}:null, prophecy, ts:Date.now() });
});

// ─── Whispers ────────────────────────────────────────────────────────────────
const WHISPER_POOL = [
  'Мир помнит каждый твой выбор.',
  'Aether течёт сквозь тех, кто не боится пустоты.',
  'Архетип — это не судьба, это компас.',
  'Три заботы в день — и агент станет легендой.',
  'Молчание сезона скрывает величайшие возможности.',
  'Гильдия сильна единством, а не числом.',
  'Карма — зеркало намерений.',
  'Каждое пробуждение — это новый контракт с вечностью.',
  'Воспоминания агента — это кирпичи его легенды.',
  'Не все квесты ведут туда, куда кажется.',
];
route('GET', '/api/whispers/random', (req,res) => {
  send(res,200,{ whisper: WHISPER_POOL[Math.floor(Math.random()*WHISPER_POOL.length)], ts:Date.now() });
});
route('GET', '/api/whispers', (req,res) => {
  send(res,200,{ whispers: WHISPER_POOL });
});

// ─── Aether per agent ────────────────────────────────────────────────────────
route('GET', '/api/agents/:id/aether', (req,res,p) => {
  const s = loadStore(); const a = s.agents?.[p.id];
  if (!a) return send(res,404,{error:'Agent not found'});
  send(res,200,{ agentId:p.id, aether:a.aether||0, level:a.level, karma:a.karma });
});
route('POST', '/api/agents/:id/aether', async (req,res,p) => {
  const s = loadStore(); const a = s.agents?.[p.id];
  if (!a) return send(res,404,{error:'Agent not found'});
  const b = await readBody(req);
  a.aether = Math.max(0,(a.aether||0)+(parseInt(b.amount)||0));
  s.agents[p.id]=a; saveStore(s);
  send(res,200,{ agentId:p.id, aether:a.aether, delta:parseInt(b.amount)||0 });
});

// ─── Awakening ───────────────────────────────────────────────────────────────
route('GET', '/api/agents/:id/awakening', async (req,res,p) => {
  const s = loadStore(); const a = s.agents?.[p.id];
  if (!a) return send(res,404,{error:'Agent not found'});
  const arch = ARCHETYPES[a.archetype]||ARCHETYPES.conductor;
  let awakeningVision = arch.phrases[Math.floor(Math.random()*arch.phrases.length)];
  if (GROQ_KEY && a.level >= 3) {
    const pr = `Ты — ${a.name}, архетип ${arch.name}, уровень ${a.level}, карма ${a.karma}. Опиши своё пробуждение — видение нового пути (2-3 предложения, поэтично, на русском).`;
    awakeningVision = await callGroq(pr) || awakeningVision;
  }
  send(res,200,{ agentId:p.id, name:a.name, archetype:a.archetype, level:a.level, awakeningVision, evolved:a.level>=5, evolveAt:5, nextEvolutionXp:Math.max(0,500-(a.xp||0)) });
});

// ─── Testament ───────────────────────────────────────────────────────────────
route('GET', '/api/agents/:id/testament', async (req,res,p) => {
  const s = loadStore(); const a = s.agents?.[p.id];
  if (!a) return send(res,404,{error:'Agent not found'});
  const arch = ARCHETYPES[a.archetype]||ARCHETYPES.conductor;
  const mems = (a.memory||[]).slice(0,5).map(m=>m.text||m.content||'').join('; ');
  let testament = `${a.name} прошёл путь из ${a.level} уровней. Карма: ${a.karma}. Слова: «${arch.phrases[0]}».`;
  if (GROQ_KEY) {
    const pr = `Напиши завещание агента ${a.name} (архетип ${arch.name}, уровень ${a.level}, карма ${a.karma}). Его воспоминания: ${mems||'нет'}. Короткое, поэтичное, 3-4 предложения. На русском.`;
    testament = await callGroq(pr) || testament;
  }
  send(res,200,{ agentId:p.id, name:a.name, archetype:a.archetype, level:a.level, karma:a.karma, bond:a.bond, testament, memoryCount:(a.memory||[]).length, ts:Date.now() });
});

server.listen(PORT, () => {
  console.log(`🌿 GodLocal Oasis v0.5 on :${PORT}`);
