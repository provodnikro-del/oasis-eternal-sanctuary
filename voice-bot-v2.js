 ...
 *
 * Новое в v2:
 *   - Inline keyboards (быстрые архетипы, голос/текст, брифинг)
 *   - /brief — голосовой утренний брифинг
 *   - /x100start — старт 100-дневной программы
 *   - /remember — сохранение заметок голосом и текстом
 *   - SQLite persistent sessions (память после рестарта)
 *   - Multi-chunk voice (длинные ответы → несколько аудио)
 *   - Callback query handler (нажатия inline кнопок)
 *
 * Env vars: BOT_TOKEN, GEMINI_API_KEY, ELEVENLABS_KEY (optional), APP_URL, WEBHOOK_SECRET
 */
...
async function handleCallback(cb) {
  ...
  if (data.startsWith('arch:')) {
    ...
    if (ARCHETYPES[arch]) {
      ...
      const { synthesize } = require('./voice-agent-v2');
  ...
  if (data === 'action:brief') {
    ...
    const { synthesize } = require('./voice-agent-v2');
    const text = await generateBriefing(session).catch(() => '⚡ Брифинг временно недоступен.');
  ...
  try {
    if (isVoice) {
      ...
      if (result.transcription) {
        await sendMsg(chatId, `🎤 _"${result.transcription}"_`, { parse_mode: 'Markdown' });
      }
    ...
    if (isText) {
      ...
      if (text === '/reset') {
        const { sessions } = require('./voice-agent-v2');
        if (sessions && sessions.delete) sessions.delete(chatId);
        await sendMsg(chatId, '🔄 Память сброшена. Начинаем заново.', { reply_markup: mainKeyboard(session) });
        return;
      }
...
const server = http.createServer(async (req, res) => {
  if (req.method === 'GET' && req.url === '/health') {
    ...
    return res.end(JSON.stringify({
      status: 'ok', bot: '@Provodnikro_bot', version: '2.0.0',
